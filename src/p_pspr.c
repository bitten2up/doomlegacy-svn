...
//-----------------------------------------------------------------------------
//
// $Id: p_pspr.c 1677 2024-03-03 04:42:42Z wesleyjohnson $
//
...
static
void P_SetPsprite_ptr ( player_t*   player,
                        pspdef_t*   psp,
                        statenum_t  stnum )
{
    state_t*    state;
                    ...
                    statenum_t    stnum )
{
    ...
    P_SetPsprite_ptr ( player, psp, stnum );
}
...
#ifdef MBF21
        if ( !player->attackdown  // autofire repeat
...
signed_angle_t MBF21_P_Random_hitscan_angle( fixed_t spread )
{
    ...
    int64_t spread_bam = ((spread < 0) ? FixedToAngle_EE(-spread) : FixedToAngle_EE(spread));
...
{
    mobj_t * pmo = player->mo;
    state_ext_t * sep = P_state_ext( pmo->state );
    if( sep == &empty_state_ext )
        return;  // no args

    {
        int p_type = sep->args[0] - 1;
        signed_angle_t  p_angle = (((int64_t)sep->args[1]) << FRACBITS) / 360;
...
//    if( damagemod == 0 )   damagemod = 10;
    int bzkmult = sep->args[2];
    int hitsound = sep->args[3];

    int range = sep->args[4];
    if( range == 0 )
...
//	return;
    ...
    S_StartMobjSound( sep->args[1] ? NULL : pmo, sep->args[0] );
}
...
//	return;
    ...
    if( PP_Random(pr_mbf21) < sep->args[1] )
      P_SetPsprite_ptr( player, psp, sep->args[0] );
}
...
//	return;
    ...
    int ammo = player->ammo[p_type];
    player->ammo[p_type] = (p_ammo_used <= ammo)? (ammo - p_ammo_used) : 0;
}
...
//	return;
    ...
    if( player->ammo[p_type] < p_ammo_req )
      P_SetPsprite_ptr( player, psp, sep->args[0] ); // to new state
}
...
//	return;
    ...
    if( (sep->args[1] || P_CheckAmmo(player))
...
//	return;
    ...
    P_SetPsprite( player, PS_flash, sep->args[0] );
}
