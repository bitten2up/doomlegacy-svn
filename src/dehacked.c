...
//-----------------------------------------------------------------------------
//
// $Id: dehacked.c 1683 2024-03-20 22:51:04Z wesleyjohnson $
//
...
byte deh_detected;  // [WDJ] bits, bdtc_game_detection_e
...
deh_fix_args_t  deh_fix_args_table[] =
{
    ...
    {{A_Spawn_MBF}, (DFAF_thing_id | 0), 0, 0, {0, 0, 0} },  // Spawn object, parm1, parm2
      // args[0] (parm1) is thing_id
    {{A_SpawnObject_MBF21}, (DFAF_thing_id | 0), 0, 0, {0, 0, 0} },  // Spawn object, args[0..7]
      // args[0] is thing_id
      // default {0}
    {{A_MonsterProjectile_MBF21}, (DFAF_thing_id | 0), 0, 0, {0, 0, 0} },  // Monster projectile attack, args[0..4]
      // args[0] is thing_id
      // default {0}
    {{A_MonsterBulletAttack_MBF21}, 0,  2, 3, {1, 3, 5}},  // Monster bullet attack, args[0..4]
      // default {0, 0, 1, 3, 5}
    {{A_MonsterMeleeAttack_MBF21}, 0, 0, 2, {3, 8, 0}},  // Monster melee attack, args[0..3]
      // default {3, 8, 0, 0}
    {{A_WeaponProjectile_MBF21}, (DFAF_thing_id | 0), 0, 0, {0, 0, 0}},  // Player projectile attack, args[0..4]
      // args[0] is thing_id
      // default {0}
    {{A_WeaponBulletAttack_MBF21}, 0, 2, 3, {1, 5, 3}},  // Player bullet attack, args[0..4]
      // default {0, 0, 1, 5, 3 }
    {{A_WeaponMeleeAttack_MBF21}, 0, 0, 3, {2, 10, 1*FRACUNIT}},  // Player melee attack, args[0..4]
      // default {2, 10, 1*FRACUNIT, 0, 0 }
    {{A_AddFlags_MBF21}, (DFAF_mobjflags | 0), 0, 0, {0, 0, 0} },  // Add flags to mobj, args[0..1]
      // args[0] are mobj flags
      // args[1] are MBF21 flags => args[1], args[7]
    {{A_RemoveFlags_MBF21}, (DFAF_mobjflags | 0), 0, 0, {0, 0, 0} },  // Add flags to mobj, args[0..1]
      // args[0] are mobj flags
      // args[1] are MBF21 flags => args[1], args[7]
    {{A_JumpIfFlagsSet_MBF21}, (DFAF_mobjflags | 1), 0, 0, {0, 0, 0} },  // Add flags to mobj, args[0..2]
      // args[1] are mobj flags
      // args[2] are MBF21 flags => args[2], args[7]
};
byte num_deh_fix_args = sizeof(deh_fix_args_table) / sizeof(deh_fix_args_t);


void fix_action_args( actionf_t action1, state_t * st )
{
    state_ext_t * sep;
    ...
    {
        // [WDJ] all types of function ptrs are comparable.
        // Must select something from union to satisfy compiler.
        if( dfap->action.acv == action1.acv )  // match
            goto match_action;
    }
    return;
...
match_action:
    ...
    if( dfap->num_default_args )
    {
        ...
        for( i=0; i< dfap->num_default_args; i++, aj++ )
        {
            ...
            if( (sep->args[aj] == 0) && !(sep->args_set & (1<<i)) )
            {
//		sep->args_set |= (1<<i);  // default only once
                sep->args[aj] = dfap->def_args[i];
            }
        }
    }

    if( dfap->fix_flags & DFAF_thing_id )
    {
        // must ensure this is done only once
        // Translate DEH thing_id to DoomLegacy thing id.
        // This is always arg[0].
        sep->args[0] = translate_thing_id( sep->args[0], "" );
    }

    if( dfap->fix_flags & DFAF_mobjflags )
    {
        int i2 = dfap->fix_flags & DFAF_args_select_mask;
        sep->args[ i2 ] = convert_std_flags_to_DL2( sep->args[ i2 ] );  // MF_
        // The mbf21 flags are in MF2_ and MF3_ in DoomLegacy.
        uint32_t mbf21_flags = sep->args[ i2+1 ];
        sep->args[ i2+1 ] = convert_MBF21_flags_to_DL2( mbf21_flags );  // MF2_
        sep->args[ 7 ] = convert_MBF21_flags_to_DL3( mbf21_flags );  // MF3_
    }
...
void fix_action_parm( actionf_t action1, state_t * st )
{
    if( action1.acv == A_Spawn_MBF )  // match
    {
        state_ext_t * sep;
        if( st->state_ext_id )
        {
...
void read_thing( myfile_t * f, int deh_thing_id )
{
  ...
  do{
    if(myfgets(s,sizeof(s),f)!=NULL)  // get line
    {
      ...
      if( explicit_mbf21_bits || !strcasecmp(word,"Bits"))
#else	
...
#endif
      {
          ...
          {
              word = strtok(NULL, " +|\t=\n");
              if( word == NULL )  goto set_flags_with_clear;
              if( word[0] == '\n' )   goto set_flags_with_clear;
              ...
              if( isdigit( (unsigned char)word[0] ) || word[0] == '-' )
              {
                  ...
                  if( explicit_mbf21_bits )
                  {
                      ...
                      // OR of flags2 and flags3 to mip flags is done in next_line
      ...
      else if(!strcasecmp(word,"fast speed"))
      {
          ...
          deh_alt_info_t * dap = create_alt_info( deh_thing_id );
          if( dap )
          {
              dap->ma_flags |= MAI_alt_speed_deh;  // MBF21 altspeed
              dap->speed[0] = mip->speed;  // normal speed
              dap->speed[1] = value;  // fast speed
          }
      }
      // melee range
...
void read_frame( myfile_t* f, int deh_frame_id )
{
  ...
  do{
    if(myfgets_nocom(s,sizeof(s),f)!=NULL)
    {
...
#ifdef MBF21
        if( word2[0] == '1' ) sep->args[0] = value;  // parm1
        else if( word2[0] == '2' ) sep->args[1] = value; // parm2
        sep->args_set |= 1;  // no defaulting
#else
      ...
      {
          // MBF21 uses these for parameters (parm1, parm2, parm3, parm4, parm5)
          // Syntax: Args4 = <num>
          ...
          if( an >='1' && an <= '8' )
          {
              ...
              sep->args[ai] = value; // 32 bit flags
              sep->args_set |= 1<<ai;  // set by DEH, for defaulting
          }
          else
          {
     argerr:
...
bex_codeptr_t  bex_action_table[] = {
   ...
   {"Detonate", {A_Detonate_MBF}},  // Radius damage, variable damage
   {"Mushroom", {A_Mushroom_MBF}},  // Mushroom explosion
   {"Die", {A_Die_MBF}},  // MBF, kill an object
   {"Spawn", {A_Spawn_MBF}},  // SpawnMobj(x,y, parm2, parm1-1)
   {"Turn", {A_Turn_MBF}},  // Turn by parm1 degrees
   {"Face", {A_Face_MBF}},  // Turn to face parm1 degrees
   {"Scratch", {A_Scratch_MBF}},  // Melee attack
   {"PlaySound", {A_PlaySound_MBF}},  // Play Sound parm1
      // if parm2 = 0 then mobj sound, else unassociated sound.
   {"RandomJump", {A_RandomJump_MBF}},  // Random transition to mobj state parm1
      // probability parm2
   {"LineEffect", {A_LineEffect_MBF}},  // Trigger line type parm1, tag = parm2

   {"KeepChasing", {A_KeepChasing_MBF}},  // MBF, from EnternityEngine
   
#ifdef MBF21
   // [WDJ] MBF21 function ptrs, from MBF21 spec and DSDA-Doom.
   ...
   {"SpawnObject", {A_SpawnObject_MBF21}},  // Spawn object, args[0..7]
      // args[0] is thing_id
      // default {0}
   {"MonsterProjectile", {A_MonsterProjectile_MBF21}},  // Monster projectile attack, args[0..4]
      // args[0] is thing_id
      // default {0}
   {"MonsterBulletAttack", {A_MonsterBulletAttack_MBF21}},  // Monster bullet attack, args[0..4]
      // default {0, 0, 1, 3, 5}
   {"MonsterMeleeAttack", {A_MonsterMeleeAttack_MBF21}},  // Monster melee attack, args[0..3]
      // default {3, 8, 0, 0}
   {"WeaponProjectile", {A_WeaponProjectile_MBF21}},  // Player projectile attack, args[0..4]
      // args[0] is thing_id
      // default {0}
   {"WeaponBulletAttack", {A_WeaponBulletAttack_MBF21}},  // Player bullet attack, args[0..4]
      // default {0, 0, 1, 5, 3 }
   {"WeaponMeleeAttack", {A_WeaponMeleeAttack_MBF21}},  // Player melee attack, args[0..4]
      // default {2, 10, 1*FRACUNIT, 0, 0 }
   {"WeaponSound", {A_WeaponSound_MBF21}},  // args[0..1]
   {"WeaponAlert", {A_WeaponAlert_MBF21}},  // args none
   {"WeaponJump", {A_WeaponJump_MBF21}},  // args[0..1]

   {"NoiseAlert", {A_NoiseAlert_MBF21}},  // Alert nearby monsters, args none
   {"HealChase", {A_HealChase_MBF21}},  // Heal monster, args[0..1]
   {"SeekTracer", {A_SeekTracer_MBF21}},  // Seeker missile, args[0..1]
   {"ClearTracer", {A_ClearTracer_MBF21}},  // Clear target, args none
   {"FindTracer", {A_FindTracer_MBF21}},  // Find target, args[0..1]
      // default {0, 10}
   {"HealChase", {A_HealChase_MBF21}},  // args[0..1]
   {"RadiusDamage", {A_RadiusDamage_MBF21}},  // args[0..1]
   {"CheckAmmo", {A_CheckAmmo_MBF21}},  // args[0..1]
   {"ConsumeAmmo", {A_ConsumeAmmo_MBF21}}, // args[0]
   {"RefireTo", {A_RefireTo_MBF21}},  // args[0..1]
   {"GunFlashTo", {A_GunFlashTo_MBF21}},  // args[0..1]

   {"JumpIfHealthBelow", {A_JumpIfHealthBelow_MBF21}},  // To state, if health below arg[1], args[0..1]
   {"JumpIfTargetInSight", {A_JumpIfTargetInSight_MBF21}},  // To state, if target in line-of-sight, args[0..1]
   {"JumpIfTargetCloser", {A_JumpIfTargetCloser_MBF21}},  // To state, if target closer than args[1], args[0..1]
   {"JumpIfTracerInSight", {A_JumpIfTracerInSight_MBF21}},  // To state, if tracer in line-of-sight, args[0..1]
   {"JumpIfTracerCloser", {A_JumpIfTracerCloser_MBF21}},  // To state, if target closer than args[1], args[0..1]
   {"JumpIfFlagsSet", {A_JumpIfFlagsSet_MBF21}},  // To state, if all arg thing flags are set, args[0..2]
   {"AddFlags", {A_AddFlags_MBF21}},  // Set thing flags, args[0..1]
   {"RemoveFlags", {A_RemoveFlags_MBF21}},  // Remove thing flags, args[0..1]
#endif
...
void read_ammo( myfile_t * f, int num )
{
  ...
  do{
    if(myfgets(s,sizeof(s),f)!=NULL)
    {
      ...
      else if(!strcasecmp(word,"Perweapon")) WeaponAmmo_pickup[num] = 2*value; 
...
void DEH_Finalize( void )
{
    ...
    {
        state_t * st = & states[i];
        if( st->action.acv )  // any ptr value
        {
#ifdef MBF21
