...
//-----------------------------------------------------------------------------
//
// $Id: p_enemy.c 1683 2024-03-20 22:51:04Z wesleyjohnson $
//
...
//        sector = actor->subsector->sector;
    ...
    for ( ; ; lastlook = (lastlook+1)&PLAYERSMASK )
    {
        ...
        // New target found
        SET_TARGET_REF( actor->target, player->mo );
...
boolean PIT_VileCheck (mobj_t*  thing)
{
    ...
    if( thing->z > (vilecheck_pos.z + vilecheck_actor_info->height)
        || thing->z < (vilecheck_pos.z - 20*FRACUNIT) )  // reasonable reach down
        return true;
#else
...
//	return;
    ...
    int p_type = sep->args[0] - 1;  // object ID, 1..
    if( p_type < 0 )
        return;

    // angle degrees
    signed_angle_t angle = (angle_t)(((int64_t)sep->args[1] << 16) / 360);
    // position as fixed_t, relative to actor
    fixed_t p_ofs_x = sep->args[2];
    fixed_t p_ofs_y = sep->args[3];
    fixed_t p_ofs_z = sep->args[4];
    // velocity (mom) as fixed_t
    fixed_t p_vel_x = sep->args[5];
    fixed_t p_vel_y = sep->args[6];
    fixed_t p_vel_z = sep->args[7];
...
//	return;
    ...
    int p_type = sep->args[0] - 1;
    if( p_type < 0 )
      return;

    // angle degrees
    signed_angle_t  p_angle = (angle_t)(((int64_t)sep->args[1] << FRACBITS) / 360);
    int p_pitch = sep->args[2];
    int p_spawn_ofs_xy = sep->args[3];
    int p_spawn_ofs_z = sep->args[4];
...
//	return;
    ...
    int hspread    = sep->args[0];
    int vspread    = sep->args[1];
    int numbullets = sep->args[2];
    int damagebase = sep->args[3];
//    if( damagebase == 0 )   damagebase = 3;
    int damagemod  = sep->args[4];
//    if( damagemod == 0 )   damagemod = 5;
...
//	return;
    ...
    int damagebase = sep->args[0];
//    if( damagebase == 0 )   damagebase = 3;
    int damagemod  = sep->args[1];
//    if( damagemod == 0 )   damagemod = 8;
    int hitsound   = sep->args[2];
    int range      = sep->args[3];
    if (range == 0)  range = actor->info->melee_range;

    // Specified range with radius applied.
//    range += actor->target->info->radius - (20*FRACUNIT);
    range += actor->target->info->radius - MELEERANGE;
...
//	return;
    ...
    P_RadiusAttack_MBF21( actor, actor->target, sep->args[0], sep->args[1], true );
}
...
//	return;
    ...
    int state = sep->args[0];
    int sound = sep->args[1];
...
//	return;
    ...
    angle_t threshold = FixedToAngle_EE(sep->args[0]);
    angle_t maxturnangle = FixedToAngle_EE(sep->args[1]);
...
//	return;
    ...
    // This uses FixedToAngle from EternityEngine
    angle_t fov = FixedToAngle_EE(sep->args[0]);
    int dist = sep->args[1];
...
//	return;
    ...
    // This uses FixedToAngle from EternityEngine
    int state = sep->args[0];
    angle_t fov = FixedToAngle_EE(sep->args[1]);
...
void A_JumpIfFlagsSet_MBF21( mobj_t* actor )
{
    ...
    if( ( ((~actor->flags ) & sep->args[1])
        | ((~actor->flags2) & sep->args[2]) // MBF21 flags MF2_
        | ((~actor->flags3) & sep->args[7]) // MBF21 flags MF3_
        ) == 0 )
      P_SetMobjState(actor, state);
}
...
void A_AddFlags_MBF21( mobj_t* actor )
{
    ...
    actor->flags  |= sep->args[0];
    actor->flags2 |= sep->args[1];  // MBF21_flags, MF2_
    actor->flags3 |= sep->args[7];  // MBF21_flags, MF3_
}
...
void A_RemoveFlags_MBF21( mobj_t* actor )
{
    ...
    actor->flags  &= ~(sep->args[0]);
    actor->flags2 &= ~(sep->args[1]);  // MBF21 flags, MF2_
    actor->flags3 &= ~(sep->args[7]);  // MBF21 flags, MF3_
}
