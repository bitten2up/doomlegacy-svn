...
//-----------------------------------------------------------------------------
//
// $Id: p_mobj.c 1683 2024-03-20 22:51:04Z wesleyjohnson $
//
...
void P_RipperBlood( mobj_t * ripper, mobj_t * bleeder )
{
    ...
    if( EV_blood_vanilla > 128 )
    {
        ...
        splat->re_color = bleeder->info->bloodcolor;
#endif
    }
}
...
void DemoAdapt_p_mobj( void )
{
    ...
    // P_Random hazard: cv_splats is not set by demo, and is not netvar.
    // Legacy 1.49, cv_splat values: { {0, "OFF"}, {1, "ON"}, {2, "Vanilla"}, {7, "Auto"}
    ...
    // Used in p.map, but dependent upon cv_splats, and EV_bloodsplat.con.
    EV_blood_vanilla = ((cv_splats.EV == 2) && (EV_legacy>=MBF21_LEGACY_VERSION)) ? EV_bloodsplat_con : 0;
